...
describe("WrappedTokenBridge", () => {
    ...
    beforeEach(async () => {
        ...
        const wrappedERC20Factory = await ethers.getContractFactory("WrappedERC20Burnable")
    ...
    describe("bridge", () => {
        ...
        it("reverts when insufficient allowance", async () => {
            ...
            await expect(wrappedTokenBridge.connect(user).bridge(wrappedToken.target, originalTokenChainId, amount, user.address, false, callParams, adapterParams, { value: fee })).to.be.revertedWith("ERC20: insufficient allowance")
        })
        ...
        it("burns wrapped tokens", async () => {
            ...
            // Allowance
            await wrappedToken.connect(user).approve(wrappedTokenBridge.target, amount)
