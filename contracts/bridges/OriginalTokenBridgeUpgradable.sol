...
contract OriginalTokenBridgeUpgradable is TokenBridgeBaseUpgradable {
    ...
    function __OriginalTokenBridgeBaseUpgradable_init(address _endpoint, uint16 _remoteChainId, address _weth)
        internal
        onlyInitializing
    {
    ...
    function bridge(
        ...
        (uint256 amountWithoutDustLD, uint256 dust) = _removeDust(token, balanceAfter - balanceBefore);
    ...
    function bridgeNative(
        ...
        (uint256 amountWithoutDustLD,) = _removeDust(weth, amountLD);
    ...
    function _bridge(
        ...
        _lzSend(
            remoteChainId, payload, callParams.refundAddress, callParams.zroPaymentAddress, adapterParams, nativeFee
        );
        emit SendToken(token, msg.sender, to, amountLD);
    }

    function withdrawFee(address token, address to, uint256 amountLD) public onlyOwner {
        ...
        whenNotPaused
    {
        ...
        (
            ...
            bool unwrapWeth
        ) = abi.decode(payload, (uint8, address, address, uint256, uint256, bool));
    ...
    function _amountSDtoLD(address token, uint256 amountSD) internal view returns (uint256) {
        return amountSD * LDtoSDConversionRate[token];
    }

    function _amountLDtoSD(address token, uint256 amountLD) internal view returns (uint256) {
